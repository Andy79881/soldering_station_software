/* Mais_kolben V2.6 with modifications due to different HW
  credits for original source code go to HannIO (Heise Make 5/2016)
  Project Github repository: https://github.com/ArduinoHannover/Maiskolben/
 
  modifications by A. Nestler
  
  change descriptions:
  - definitions.h changed to different HW (pin usage)
  - TFT_BL added in setup routine (set to output and High)
...
// fucntion prototypes - needed for cpp
void setup(void);
...
void setup(void) {
	...
	digitalWrite(TFT_BL, HIGH); // activate back light
	Serial.begin(115200);
